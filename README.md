## Note the main ReStore branch is now for Dolphin Smalltalk 7.2. Please use named branches for Dolphin Smalltalk 7.1 and 7.0.
...
* To install ReStore with both SQLite and MySQL native support evaluate:
  ```smalltalk
  GitHubPackageManager install: 'rko281/ReStore/Native Connections'
  ```
* To specify a SQLite connection:
  ```smalltalk
...
* To specify a MySQL connection:
  ```smalltalk
    aReStore connection:
      (SSWMySQLConnection on:
          (MySQLDriverSpec new
            host: '192.168.1.234'; db: 'database';
